...
if [ -f "docker-compose.yml" ]; then
  ...
  # Ensure the Traefik dynamic config directory exists so the bind-mount in
  # docker-compose.yml has a real directory to attach to. The actual dynamic.yml
  # is generated by .worktree/switch-branch.sh and is gitignored.
  mkdir -p ".worktree/traefik"
  if ! grep -qF '.worktree/traefik/dynamic.yml' .gitignore 2>/dev/null; then
    echo '.worktree/traefik/dynamic.yml' >> .gitignore
    echo "[container-wt] Added .worktree/traefik/dynamic.yml to .gitignore."
  fi
fi
